...
void run_server(const char* ip, int port) {
    ...
    while (1) {
        ...
        //initialize the receiver from head packet
        while(1){
            ssize_t received = recvfrom(sock, &head, sizeof(Head), 0, (struct sockaddr*)&client_addr, &client_len);
            if (received < 0) {
                perror("recvfrom failed");
                continue;
            }
            printf("Received head packet for file %s\n", head.filename);
        ...
        // create multiple windows for multiple files
